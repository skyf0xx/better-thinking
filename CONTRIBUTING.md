...
4. **Stay in budget.** Token estimate (chars ÷ 4) is enforced: atomic skills ≤ 900 tokens, composite ≤ 1,700, dispatcher ≤ 2,000. If a skill keeps pushing past its ceiling, it's doing too much — split it.
   ...
   - **The `dispatcher` type.** [[better-thinking]] is the one skill of this type, at a 2,000-token ceiling rather than atomic's 900. It carries two jobs an ordinary atomic skill doesn't: dynamic routing logic (reading the index, matching a task to candidate skills) *and* the output-formatting contract every response in the plugin follows (inline labels, the Applied footer, the Caught line). That second job is system-wide infrastructure, not per-task cognition, and it costs real procedure text — hence a dedicated type and budget rather than stretching atomic's definition or misclassifying it as composite (it has no fixed `dependencies` to name; its whole job is deciding those dynamically per task).
