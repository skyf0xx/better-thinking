...
A recipe doc alone is enough to make a framework runnable: [[recipe-runner]] auto-detects a named framework or its stage description and executes the mapped sequence directly from the doc — no skill file required. Optionally, a frequently-reached-for recipe can also get a thin shim skill for direct slash-command invocation (e.g. `skills/better-thinking-design-thinking/SKILL.md`, `/better-thinking-design-thinking`). Always prefix these shims `better-thinking-<name>` — never the bare framework name — so every recipe shares the same discoverable stem as the dispatcher itself, rather than reading as a primitive thinking skill in a flat, alphabetical list of 130+ skills. A shim's entire procedure is "invoke `recipe-runner` fixed to this recipe"; it must not duplicate `recipe-runner`'s orchestration logic inline.
