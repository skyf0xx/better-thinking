...
- **Decide atomic vs. composite.**
  ...
  - **Dispatcher** = a third type, reserved for `better-thinking` alone (not `better-thinking-recipes`, which is `composite` — it names fixed dependencies, the union of every recipe stage's skills). A dispatcher routes dynamically to whichever skill(s) fit the incoming task rather than naming fixed dependencies, so like atomic it declares none — but it also owns a formatting/output contract that applies across every response in the plugin, which atomic's 900-token ceiling can't hold. You will not add a new `dispatcher` skill; the type is reserved for this one entry point.
...
4. **Stay in budget.** Token estimate (chars ÷ 4) is enforced: atomic skills ≤ 900 tokens, composite ≤ 1,700, dispatcher ≤ 2,300. If a skill keeps pushing past its ceiling, it's doing too much — split it.
   - **Exception: interactive orchestrators.** [[better-thinking-recipes]] is exempt from the 1,700 composite ceiling, capped instead at 2,500. Its procedure must specify checkpoint behavior (when to stop and wait for user go-ahead between stages) on top of the stage-invocation logic every composite carries, and that costs real procedure text. This exception is granted per-skill, not by category — a new composite doesn't get the higher ceiling just for being multi-stage; it has to actually orchestrate a paused, multi-turn interaction the same way.
   - **Exception: fixed enumerations.** [[multi-framework-ideation]] (atomic, capped at 1,300) and [[spread-potential-scoring]] (atomic, capped at 1,250) exceed the 900 atomic ceiling because each encodes a fixed, named enumeration — four distinct generation mechanisms, or a five-gate multiplicative rubric plus two modifiers — where every element is load-bearing and cutting any one loses part of the technique itself, not just prose. Granted per-skill in `scripts/check_consistency.py`'s `TOKEN_BUDGET_EXCEPTIONS`, sparingly: a skill earns this by actually needing the space for its specific enumeration, not by category membership or by being "detailed."
   - **The `dispatcher` type.** [[better-thinking]] is the one skill of this type, at a 2,300-token ceiling rather than atomic's 900. It carries three jobs an ordinary atomic skill doesn't: dynamic routing logic (reading the index, matching a task to candidate skills), a clarify step that resolves vague objectives/constraints before routing, *and* the output-formatting contract every response in the plugin follows (inline labels, the Applied footer, the Caught line). Those extra jobs are system-wide infrastructure, not per-task cognition, and they cost real procedure text — hence a dedicated type and a wider budget rather than stretching atomic's definition or misclassifying it as composite (it has no fixed `dependencies` to name; its whole job is deciding those dynamically per task).
